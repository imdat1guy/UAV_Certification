...
function csvField(v) {
  ...
  const s = String(v);
  // If the field contains quotes, commas, or newlines, wrap in quotes and escape quotes
  if (/[",\n\r]/.test(s)) {
    return '"' + s.replace(/"/g, '""') + '"';
  }
  return s;
...
function appendCsv(path, row) {
  ...
  const line = headers.map(h => csvField(row[h])).join(',') + '\n';
  fs.appendFileSync(path, line);
}

function serializeError(err) {
  try {
    // capture message, stack, cause, etc.
...
async function main() {
  ...
  // Ensure notified body is authorized, but avoid redundant txs
  try {
    const nbAddr = await nb.getAddress();
    const isAlready = await contractReg.notifiedBodies(nbAddr); // <- view call, no gas

    if (isAlready) {
      // Log a no-op "check" row so your runs remain auditable
      appendCsv(OUT, {
        run_id: RUN_ID, network: NETWORK, rollup_type: ROLLUP,
        workflow: 'W1', op_type: 'authorizeNotifiedBody_check', uav_count: N, concurrency: CONC,
        tx_hash: '', submitted_at_utc: nowISO(),
        included_at_utc: '', finalized_at_utc: '',
        block_number: '', confirmations: '',
        gas_used: '', effective_gas_price_wei: '',
        success: 1, error: '', notes: 'already_authorized'
      });
    } else {
      // Only send the tx if needed
      const subTS = nowISO();
      const tx = await contractReg.authorizeNotifiedBody(nbAddr);
    ...
    // If the read or tx fails, log a well-escaped error row
    appendCsv(OUT, {
      run_id: RUN_ID, network: NETWORK, rollup_type: ROLLUP,
      workflow: 'W1', op_type: 'authorizeNotifiedBody_error', uav_count: N, concurrency: CONC,
      tx_hash: '', submitted_at_utc: nowISO(),
      included_at_utc: '', finalized_at_utc: '',
      block_number: '', confirmations: '',
      gas_used: '', effective_gas_price_wei: '',
      success: 0, error: serializeError(e), notes: ''
    });
  }
    ...
    const go = (async () => {
      ...
      try { await oneWorkflow(i); }
        ...
        appendCsv(OUT, {
          ...
          op_type: 'workflow_error', uav_count: N, concurrency: CONC, success: 0, error: serializeError(e)
        });
      } finally { active--; }
    })();
    tasks.push(go);
